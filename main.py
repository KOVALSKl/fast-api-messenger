...
class ConnectionManager:
    ...
    async def send_personal_message(self, friend_login: str, message: str, websocket: WebSocket):
        try:
            ...
            await websocket.send_text(f'Пользователь {friend_login} не в сети')
...
@app.websocket('/ws/{client_login}/{user_login}')
async def websocket_endpoint(websocket: WebSocket, client_login: str, user_login: str):
    ...
    try:
        while True:
            ...
            await manager.send_personal_message(client_login, f"Client {user_login} send you: {data}", websocket)
